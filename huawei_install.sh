...
#
        ...
        [yY][eE][sS]|[yY])
            ...
            cd /$lj/ss5-3.8.9/
            ./configure >/dev/null 2>&1;make >/dev/null;make install >/dev/null 2>&1
            echo "===配置 编译  安装 ........ OK"
...
#
            ...
            case $ok0 in
                [yY][eE][sS]|[yY])
                    ...
                    if [[ $? -eq 0 ]];then
                        ...
                        echo -e "\n\n"
                        echo "未安装ss5,请重新运行 huawei_install.sh 脚本输入 Y 或 y 进行安装!!!"
                        echo "ss5 is not installed, please re-run the huawei_install.sh script and enter Y or y to install!!!"
                        echo -e "\n\n"
                    fi
        ...
        [zZ][sS][tT][aA][tT][eE]|[zZ])
            ...
            if [[ $? -eq 0 ]];then
                ...
                echo "############################################################################################################################################################"
                echo -e "\n"
                echo "                     未安装ss5或未运行ss5;请重新运行 huawei_install.sh 脚本输入 [安装编译: "Y" 或 "y" ] 或 [启动进程: "r" 或 "R" ]!!!"
                echo "ss5 is not installed or running; please re-run the huawei_install.sh script and enter [install compile: "Y" or "y" ] or [start process: "r" or "R" ]!!!"
                echo -e "\n"
                echo "############################################################################################################################################################"
                echo -e "\n"
            fi
        ...
        echo -e "\n\n"
        echo "请在华为云弹性云服务器ECS上使用 huawei_install.sh 脚本!!!"
        echo "Please use the huawei_install.sh script on HUAWEI CLOUD ECS!!!"
        echo -e "\n\n"
    fi
else
    echo -e "\n\n"
