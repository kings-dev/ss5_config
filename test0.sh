...
if [ $? -eq 0 ];then
    ...
    wget -q --no-check-certificate --no-certificate-check https://raw.githubusercontent.com/kings-dev/ss5_config/main/test1.sh -O /root/test1.sh > /dev/null && cd ~ && sh test1.sh
    echo "ping no ok"
else
    wget -q -no-check-certificate --no-certificate-check https://raw.githubusercontent.com/kings-dev/ss5_config/main/test1.sh -O /root/test1.sh > /dev/null && cd ~ && sh test1.sh
    echo "ping is ok"
